...
class Prompt:
    def __init__(self):
        self.prompt = {}  # In-memory storage for chat histories
        self.content = {}

    def getSystemPrompt(self, sessionId):
        return self.prompt[sessionId]

    def getEpisodeContent(self, sessionId):
        return self.content[sessionId]

    def initialize_prompt(self, session_id: str, name, topics):
        self.prompt[session_id] = self.modifyPrompt(name, topics)

    def modifyPrompt(self, name, main_topics):
        formatted_prompt = SYSTEM_PROMPT.format(
            episode_title=name,
            list_of_main_topics=main_topics,
        )
        print(formatted_prompt)
        return formatted_prompt

    def extract_episode_details(self, episode_data: dict):
        # Extract the episode name
        episode_name = episode_data.get("EpisodeName", "Unknown Episode Name")

        # Extract all keys except 'EpisodeName'
        main_topics = [key for key in episode_data.keys() if key != "EpisodeName"]

        return episode_name, main_topics

    def fetchEpisodeContent(self, access_token, episode_id, sessionId):
        supabaseAnonSync.postgrest.session.headers.update(
            {"Authorization": f"Bearer {access_token}"}
        )
        supabase_response = (
            supabaseAnonSync.table("episodes")
            .select("*")
            .eq("id", episode_id)
            .execute()
        )
        episodeConetent = supabase_response.data[0]["episode"]
        self.content[sessionId] = episodeConetent
        if isinstance(episodeConetent, str):
            episodeConetent = json.loads(episodeConetent)

        name, topics = self.extract_episode_details(episodeConetent)

        self.initialize_prompt(sessionId, name, topics)
